...
for aktie in meine_aktien:
    try:
        ...
        perf_m = ((raw_kurs - raw_kurs) / raw_kurs) * 100
        if len(hist_1m) > 0: perf_m = ((raw_kurs - kurs_1m) / kurs_1m) * 100
        ...
        try:
            ...
            if ex_date_raw:
                try:
                    ex_date_str = datetime.utcfromtimestamp(int(ex_date_raw)).strftime('%d.%m.%Y')
                except Exception:
                    ex_date_str = "-"
        except Exception:
            pass
